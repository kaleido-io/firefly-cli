...
type ERC721Provider struct {
	Log     log.Logger
	Verbose bool
	Stack   *types.Stack
}

func (p *ERC721Provider) DeploySmartContracts() error {
	return nil
}

func (p *ERC721Provider) FirstTimeSetup() error {
...
func (p *ERC721Provider) GetDockerServiceDefinitions() []*docker.ServiceDefinition {
	...
	for i, member := range p.Stack.Members {
		serviceDefinitions = append(serviceDefinitions, &docker.ServiceDefinition{
			...
			Service: &docker.Service{
				Image:         p.Stack.VersionManifest.TokensERC721.GetDockerImageString(),
				...
				Environment: map[string]string{
					...
					"ETHCONNECT_INSTANCE":     "/contracts/erc721Factory",
					"ETHCONNECT_TOPIC":        "tokenERC721",
...
func (p *ERC721Provider) GetFireflyConfig(m *types.Member) *core.TokensConfig {
...
func (p *ERC721Provider) getEthconnectURL(member *types.Member) string {
	return fmt.Sprintf("http://ethconnect_%s:8080", member.ID)
}

func (p *ERC721Provider) getTokensURL(member *types.Member) string {
	if !member.External {
